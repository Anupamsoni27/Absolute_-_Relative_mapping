...
obeservation_list = []
path ="C://python_tool//Absolute_Relative_mapping//"
class main():
    ...
    def method2(self, temp_date, temp_model, temp_relative_model_code):
        df2 = pd.read_excel(
            "C://python_tool//Absolute_Relative_mapping//input//file.xlsx",
            ...
            if "Date" not in t_model and str(t_model.split("/")[-3]).split(".")[-1] == temp_relative_model_code:
                ...
                for a in observation_list:
                    if "False" ==str(a):
                        ...
                        absolute_value = float(df3.iloc[0,1])
                        relative_value = float(df3.iloc[0,2])
...
global df
xl = pd.ExcelFile("C://python_tool//Absolute_Relative_mapping//input//file.xlsx")

print(xl.sheet_names)
df_rollover = pd.read_excel("C://python_tool//Absolute_Relative_mapping//input//file.xlsx",
                   sheet_name=xl.sheet_names[1], index_col=None, header=[0])

df = pd.read_excel("C://python_tool//Absolute_Relative_mapping//input//file.xlsx",
...
writer = ExcelWriter(path+'output/output.xlsx')
